...
- `Metrics.AddMetric` aggregates timing/token-count fields across stages; `StartTime`/`EndTime`/`TotalTime` track wall-clock per request. `Metrics.PerTask` breaks executions/failures/duration/LLM calls/tokens down per pipeline task id (`RecordTaskAttempt`/`RecordLLMCall`), which is what the energy evaluation sums per stage — see `evaluation/EVALUATION.md`.
