...
**I. Prediction & candidate selection** (new 2026-08-24, decisions settled the same day; implemented 2026-09-02/03 — status per item below)
...
- [x] [I1] Labelled corpus: one full `evaluation_set` pass; label = `all_tests_passed` — **use `run-20260831-170746`, 42/95 (44.2%)**; first pass 11/95 kept as provenance. **Frozen-config replicate series 20260904/0911/0912: 47/43/46, pairwise agreement 82–85%, Fleiss κ 0.68** (EVALUATION.md "Replicate series")
...
- [x] [I2] Signal check — **answered 2026-08-31 on the second pass: criterion PASSES.** Base rate 44.2%; complexity still no signal (A vs D+ p=0.37), AWS strong (27.6% vs 70.3%, p=5.5e-5). Modelling is worth doing; beat the skip-AWS baseline, not always-translate. **Not stable across pipeline versions:** on the frozen-config replicates complexity separates (A 63% vs D+ 13%, p=0.0002) and the AWS split shrinks to 42% vs 57% (n.s.)
- [x] [I4] One feature/label table every method and baseline consumes **(P0)**
- [x] [I5] Baselines: always-translate, never, `cc` threshold, infeasibility rule list, skip-AWS
- [x] [I6] Candidate methods: M1 logistic regression + M2 random forest (M3 LLM-judge not run; MLP deferred) **(P0)**
- [x] [I7] Split protocol (repeated stratified *group* k-fold, not one holdout) + net energy vs. always-translate **(P0)**
...
### [x] [C11b] Is the Floci route a source of run-to-run noise? Measured: no, for any function that would otherwise pass
- Category: Evaluation / Fault Tolerance (investigation)
- Affected component(s): `internal/floci/deployer.go` (`lambdaTimeoutSeconds`), `internal/floci/stage.go` (measured, unchanged)
- Problem / current state: the plan to average translation success over repeated `evaluation_set` runs (proposed 2026-09-11; carried out as runs 20260911-165103 and 20260912-172904 — results in EVALUATION.md "Replicate series") needs the pipeline's run-to-run variance to be *LLM sampling*, not infrastructure. Comparing run `20260831-190900` against `20260904-190539`, **23 of 95 functions flipped outcome (24%) for a net change of +5** (14 gained, 9 lost) - and 9 regressions under the `cleaner`->`summary` change ([C15]), which is neutral-to-positive per function by construction, is not plausible as a deterministic effect. The flips were more frequent on the Floci route (13/40 = 32%) than on goTester (10/55 = 18%), raising the question of whether the emulator contributes nondeterminism - in particular whether state bleeds between jobs and corrupts side-effect assertions.
- Method: replay the **archived translation** through `flociTester` repeatedly. The package from `runs/packages-<id>/<fn>.zip` is fixed bytes, so the LLM is out of the loop entirely and any variation is infrastructure. Nine functions x 3 replays: five flippers whose failure kind was emulator-suspicious (`execution error` f46/f53/f74, `side-effect mismatch` f47/f83), one further flipper (f78), two stable-pass controls (f11/f30) and one stable-fail control (f6). The throwaway harness is not committed; `floci.NewFlociTester(nil).Apply` needs only a `pipeline.Runner{Context: ...}`, since `packageLambda` builds the module itself.
- Result: **0 verdict flips in 9 functions.** 8 of 9 were deterministic at the *case* level too - identical per-case outcomes across all three replays, including f47's DynamoDB and f83's S3/SQS side-effect assertions, which were the specific worry. Failures reproduce as exactly as passes: f6 fails the same two cases with byte-identical `TypeAssertionError` panics every replay and passes the third every replay.
  - **The route difference was not significant to begin with**: 13/40 vs 10/55 is Fisher exact two-sided **p = 0.146**. An earlier note in this session called it "roughly double"; that overstated a difference of three functions against the null.
  - **The flip mechanism is the compiler, on both routes.** Classifying each flipper by what the failing run failed with: Floci 7 build failures / 2 output mismatch / 3 execution error / 1 side-effect mismatch; goTester 9 build failures / 1 execution error. The dominant event is the LLM emitting code that compiles on one sampling and not another - which is exactly what replicate runs are meant to average, and it does average.
  - **No flipper failed with `setup failed`**, the kind that would most directly indicate provisioning flakiness.
- The one nondeterministic function, and why it does not matter: f46's case outcomes oscillate per deployment (all 5 cases time out in one deployment, 3 pass in the next) across three separate sessions, warm emulator or cold. But **every function whose translation was correct was perfectly deterministic**, and every function that showed churn also had a genuine panic-inducing defect - f46 a `TypeAssertionError` at `main.go:97`, f6 the same, f63 seven `reflect.Value.Convert: value of type string cannot be converted to type types.AttributeValue`. The churn therefore lands only where the verdict is already fixed by a real defect, which is why it produced no verdict flip.
- **Negative result: raising `lambdaTimeoutSeconds` does not fix it, and the constant was reverted to 30 s.** The hypothesis was that a 30 s budget sat too close to the emulator's slow path, and that the resulting spurious `execution error`s biased the success rate downward (a spurious timeout converts a pass to a failure, never the reverse). Measured at 60 s: f46 produced **7 x "Task timed out after 60 seconds"** and f63 **8**, with unchanged outcomes. The timeouts co-occur with handler panics, which is consistent with a panicking handler leaving its container unable to answer the next invocation - a dead container does not finish sooner given more time. Doubling the budget only doubles the wait on an already-failing function (~15 min per run, at run `20260904-190539`'s 31 timeout occurrences), so it was reverted with the finding recorded at the constant.
- Consequence for the measured bias: the downward bias first estimated at "at most 3 of 95" (f15/f35/f63, whose cases all failed with `execution error`) is smaller still, because **f63 was shown to fail deterministically** for genuine `AttributeValue` type errors. It is not a spurious timeout victim. f6, f46 and f48 would fail without their timeouts too (independent panics; f48 fails on an `output mismatch`).
- Why this matters: it clears the Floci route as a confound for those replicate runs. The 24% churn is LLM sampling and will average; the emulator adds nothing to functions that would otherwise pass. It also means a function labelled "flaky" by repeated runs is flaky because of the model, not the harness - which is what makes label stability reportable as a result rather than an artifact.
- Not fixed here, and probably the real fix if the timeouts are ever worth removing: treat a **wholesale timeout of every case in one deployment** as an infrastructure signal and redeploy once, rather than recording it as a test result. A per-case retry would not help, since the container rather than the case is what is wedged.
- Architecture impact: None (investigation; one constant left unchanged) | Effort: S | Priority: P2
...
- Scale, re-costing run 20260831-190900 with a nominal 25 W / 11 W idle: mean `E_translation` **11.5 kJ → 16.1 kJ** per completed translation (+40%), of which ~2.6 kJ is marginal. The measurement then showed that nominal wattage to be roughly 20× too high: measured host energy is 1.4–2.4% of run spend (idle 0.37–0.40 W, run average 0.9–1.2 W) — small, and it needed measuring to know that.
...
- **Second-pass measurement 2026-09-02 (`runtime-20260831-190900.json`, promoted to `evaluation/runtime.json`) — this is the measurement to quote.** Run against [I1]'s second-pass packages (`packages-20260831-190900.zip`), same host, `performance` governor, on AC, `rapl`/`package-0`. **66 of 95 measured** (against 57 on the first pass), 23 provisioned through fixture setup, 1 killed by the new [H10] timeout and reported `TIMEOUT`.
  ...
  | `N*` computed for | 8 | **25** |
  | `N*` median | 7.76e6 | **1.23e7** (first written as 1.17e7, without PUE) |
  ...
  - **Payback distribution over the 42 successes**: within 100k invocations 7 (17%), within 1M 11 (26%), within 10M 12 (29%), within 100M 21 (50%); **17 (40%) never pay back at all** because Go is not faster at steady state. Cheapest: f76 (15.7k), f77 (28.2k), f30 (36.8k), f45 (58.0k), f40 (90.6k) — recomputed with PUE; the counts are unchanged.
  - **Correction 2026-09-14: the `measured`, `steady state` and `cold start` rows of the table above, and the AWS/bucket speedups in the next bullet, were computed over all 66 measured packages, 24 of which failed their fixtures** (see EVALUATION.md §6 on `-runlog`). Over the 42 validated translations the medians are **1.21x steady-state energy** (1.19x wall time) and **50.5x cold-start energy**; by AWS usage **1.95x (n = 16) vs 0.98x (n = 26)**. `N*` was never affected by the filter, since it joins completed translations. Quote the frozen-config replicate series in EVALUATION.md rather than this table.
...
- **Status: SECOND pass 2026-08-31, `run-20260831-170746.jsonl` (run id `20260831-190900`) — this is the corpus to use.** 95 attempted, 0 skipped, 0 errors; **42 completed (44.2%)**, against 11 (11.6%) on the first pass. Same host, same model, same corpus; what changed was the pipeline, not the data — see the three fixes below. The first pass is kept above as provenance (and because [I2]'s superseded conclusion rests on it), but every downstream item ([I4]–[I9]) should consume this one.
  ...
  - **Energy and payback for this pass** (full report: `go run ./cmd/energy -runtime evaluation/runtime.json runs/run-20260831-170746.jsonl`):
    ...
    - **Break-even ([H6]'s second-pass measurement): median N* = 1.23e7 invocations; 17 of the 42 successes never pay back.** (`cmd/energy` on the current constants, 2026-09-14; the 1.17e7 first written here is exactly a factor 1.05 lower, i.e. it omitted PUE.) Only ~26% repay within 1M invocations. So on this corpus most successful translations are not worth performing on energy grounds - which is the [I9] argument, now resting on measured data for every success rather than on 8 functions.
...
2. **Single-run, stochastic labels.** The model's labels still come from one pass (20260831-190900), but their reproducibility is **no longer unmeasured**: three full runs of the frozen configuration (2026-09-04/11/12) agree pairwise on 82–85% of functions (Fleiss κ 0.68; 23 of 95 change outcome at least once). Report accuracy figures against that. Scored against those three runs the shipped M1 reaches AUC 0.78 / 0.70 / 0.74.
